...
function init_database() {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS `lottery_prob` (
        `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    ...
    $pdo->exec("INSERT IGNORE INTO lottery_prob (prob) VALUES (10.00)");
    $pdo->exec("INSERT IGNORE INTO categories (slug, name, sort_order) VALUES
        ('market','大盘推荐',1),('sim','模拟器',2),('night','深夜福利',3)");
...
PHPCONFIG;
        ...
        $config_content = str_replace('{{DB_HOST}}', addslashes($db_host), $config_content);
        $config_content = str_replace('{{DB_NAME}}', addslashes($db_name), $config_content);
        $config_content = str_replace('{{DB_USER}}', addslashes($db_user), $config_content);
        $config_content = str_replace('{{DB_PASS}}', addslashes($db_pass), $config_content);
        $config_content = str_replace('{{ADMIN_USER}}', addslashes($admin_user), $config_content);
        $config_content = str_replace('{{ADMIN_HASH}}', password_hash($admin_pass, PASSWORD_DEFAULT), $config_content);

        $saved = file_put_contents(__DIR__ . '/config.php', $config_content);
        if ($saved === false) {
            $errors[] = '写入 config.php 失败，请检查 admin 目录权限';
        }
    }
...
.logo h1 { font-size: 22px; font-weight: 700; color: #fff; letter-spacing: 0.05em; }
...
.step.done .step-num { background: #c9a227; border-color: #c9a227; color: #fff; }
.step.active .step-num { background: #c9a227; border-color: #c9a227; color: #fff; box-shadow: 0 0 0 4px rgba(201,162,39,0.2); }
.step-label { font-size: 11px; color: #555; }
.step.active .step-label { color: #c9a227; }
.step.done .step-label { color: #888; }
h2 { font-size: 17px; font-weight: 600; margin-bottom: 4px; color: #fff; }
...
.btn {
    ...
    font-family: inherit; transition: opacity 0.2s; margin-top: 8px;
}
.btn:hover { opacity: 0.9; }
.alert { padding: 12px 16px; border-radius: 10px; font-size: 13px; margin-bottom: 20px; line-height: 1.6; }
.alert-error { background: rgba(255,77,79,0.12); border: 1px solid rgba(255,77,79,0.3); color: #ff7875; }
.alert-info { background: rgba(201,162,39,0.1); border: 1px solid rgba(201,162,39,0.3); color: #e8c55a; }
...
.success-icon {
    ...
    border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    margin: 0 auto 16px; font-size: 32px; color: #52c41a;
}
...
.info-table td { padding: 8px 12px; font-size: 13px; border-bottom: 1px solid rgba(255,255,255,0.05); }
.info-table td:first-child { color: #666; width: 90px; }
.info-table td:last-child { color: #e0e0e0; font-family: monospace; word-break: break-all; }
.divider { height: 1px; background: rgba(255,255,255,0.06); margin: 20px 0; }
.link-btn {
    display: inline-block; padding: 10px 24px;
    background: #c9a227; color: #fff; text-decoration: none;
    border-radius: 10px; font-size: 14px; font-weight: 600;
    margin-top: 16px; transition: opacity 0.2s;
}
.link-btn:hover { opacity: 0.85; }
.note { font-size: 11px; color: #444; margin-top: 16px; text-align: center; line-height: 1.6; }
</style>
</head>
<body>
<div class="wrap">
